...
const ReviewPreview = ({ review }) => {
    ...
    return (
        <div className='review-card' style={{ borderColor: review.rating > 2 ? review.rating > 3 ? reviewColors[1] : reviewColors[0] : reviewColors[2] }}>
            <div>
                {review.user.photo && <Avatar size={80} style={{ backgroundColor: review.user ?? '#87d068' }} src={`${utils.apiHOST}images/users/${review.user.photo}`} />}
                {!review.user.photo && <Avatar size={80} style={{ backgroundColor: review.user ?? '#87d068' }}>{review.user.firstname[0]}{review.user.lastname[0]}</Avatar>}
            </div>
